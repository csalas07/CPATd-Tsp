...
float beta;
int *perms;
int ***records; //seria bueno usar el 3d, aunque quiza por separado sea comodo los arreglos de memoria compartida
#pragma endregion
...
#pragma region Proceso de instancias
	...
	for (l = 0; l < Nfiles; l++) {
		...
		//printValues();
		//lowerBound = GetLowerBoundHeuristic(); //Inicio del lowerbound

		do { //Genera el LowerBound
			iterSinMejora--;
			GRASP(&perms, &lc); //Generar una permutación con GRASP
			ov = CalcOv(perms);	//Calculo de la funcion objetivo
			if (ov < bestOv){
				bestOv = ov;
				iterSinMejora = 10;
			}
			EraseGrasp(&perms, &lc);		//Limpiamos perm y lc
...
float GetLowerBoundHeuristic() //Mover para main
{
	//int* perms, i;
	//float** lc;
	float ov, bestOv = FLT_MAX;
	int i, iterSinMejora = 10;

	//perms = new int[n];
	//lc = Memory::ReseveBiVectorflt(n, 4); //Generar memoria para lc
	...
	/*for (i = 0; i < n; i++) { //borramos lc
		delete[] lc[i];
	}*/
	//delete[] lc; //borramos lc
	//delete[] perms; //borramos permutaciones
	return (bestOv);
}
...
void GRASP(int **perms, float ***lc) {
	...
	(*lc)[Valrand][lcs::en] = 0; //Ya fue seleccionado
	(*lc)[Valrand][lcs::set] = 1;
	(*lc)[Valrand][lcs::ad] = 0;
	(*lc)[Valrand][lcs::c] = 0;
	miss--;
	prev = (*perms)[0];
